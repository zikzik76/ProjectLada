...
class c_tpa_01_form_keanggotaan  extends CI_Controller {
  ...
  //       $this->load->helper(array('url','form'));
		...
		// $this->regist_anggota_profile($_SESSION);
		
		// print_r($_SESSION['val']['username']);
		// exit();
		// $this->load->model('m_tpa_01_query_flaguser');
		// $flaguser = $this->m_tpa_01_query_flaguser->get_flaguser($_SESSION['val']['username']);
        ...
        $appstat = $this->m_tpa_01_query_appstat->approval_status($_SESSION['val']['username']);
         ...
         // $this->load->model('m_tpa_01_query_appstat');
	    // $appstat = $this->m_tpa_01_query_appstat->approval_status($var['username']);
	...
	public function regist_anggota_profile(){
		// print_r($this->input->post('optradio_anggota'));
		// exit();
	session_start();

	// $this->form_validation->set_rules('negara_asal', 'negara_asal', 'trim|required');
	// $this->form_validation->set_rules('n_calon', 'n_calon', 'trim|required');
	// if (empty($_FILES['ak_pen']['name']))
	// {
	//     $this->form_validation->set_rules('ak_pen', 'Document', 'required');
	// }
	// exit();
	...
	// {
	// 	echo "<script>alert('Please Fill Data And Document to Complete Data');</script>";
