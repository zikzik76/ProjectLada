...
class m_tpa_01_query_upload_app_form extends CI_Model 
{
	...
	public function update_tradeprogress_app_form($exchange, $urlpath)
	{
	       ...
	       $db_host = 'KBIDEV-TIMAH-DBMS';
              ...
              $exchange = $db->escape($exchange);
                $urlpath = $db->escape($urlpath);

                $query = "UPDATE SKD.EODTradeProgress SET AppFormPath = '".$urlpath."', AppStatus = 'P', AppApproveDate = GETDATE() WHERE ExchangeRef = '".$exchange."'";
